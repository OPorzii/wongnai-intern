...
public class InvertedIndexMovieSearchService implements MovieSearchService {
    ...
    public List<Movie> search(String queryText) {
        ...
        String[] querySplited = queryText.toLowerCase().split("\\s+");
        Map<String, HashSet<Long>> movieIndexList = getMovieIndexList();
        boolean wordNotMatch = false;

        HashSet<Long> selectedMovies = new HashSet<>();
        for (String w : querySplited) {
            if(movieIndexList.containsKey(w)) {
                if (selectedMovies.isEmpty())
                    selectedMovies = movieIndexList.get(w);
                selectedMovies.retainAll(movieIndexList.get(w));
            } else {
                wordNotMatch = true;
                break;
            }
        }
        List<Movie> result = wordNotMatch ? Collections.EMPTY_LIST : new ArrayList<>(movieRepository.findByIdIsIn(selectedMovies));
        return result;
    }

    private Map<String, HashSet<Long>> getMovieIndexList() {
        Map<String, HashSet<Long>> movieIndexList = new HashMap<>();
        for (Movie item : movieRepository.findAll()) {
            String[] wordName = item.getName().toLowerCase().split("\\s+");
            for (String n : wordName) {
                if (!movieIndexList.containsKey(n))
                    movieIndexList.put(n, new HashSet<Long>(Arrays.asList(item.getId())));
                movieIndexList.get(n).add(item.getId());
            }
        }
